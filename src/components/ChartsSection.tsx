/**
 * ChartsSection.tsx — AutoTrendX Live Chart Engine
 *
 * Architecture rules enforced here:
 *  1. ALL WebSocket logic lives inside useEffect — zero server-side code.
 *  2. Zero iframes. Native <canvas> only.
 *  3. Exact broker tick-subscription payload: { ticks, subscribe: 1 }
 *  4. Asset-switch cleanup sends forget_all:"ticks" then closes socket gracefully.
 *  5. Broker errors surface with their exact code + message in console.error.
 */
...
// ─── WebSocket endpoints ──────────────────────────────────────────────────────
// RULE 1: Direct browser → broker. No Vercel proxy.
...
const MAX_TICKS     = 300; // rolling live-tick buffer
const HISTORY_COUNT = 200; // historical ticks preloaded on connect

// ─── Safari / old-Chromium roundRect polyfill ─────────────────────────────────
...
function MRow({
  ...
  return (
    ...
    >
      ...
      <span className={`flex-1 text-xs truncate ${active ? 'font-bold text-gray-900' : 'text-gray-700'}`}>
        {m.name}
      </span>
      <button
...
// RULE 2: Pure <canvas> — zero iframes, zero embeds.
function ChartCanvas({
  ...
  const draw = useCallback(() => {
    ...
    // Price domain with 8% padding so the line never clips to the edge
    ...
    if (chartType === 'area') {
      ...
      grad.addColorStop(0,   isUp ? 'rgba(0,217,126,0.20)'  : 'rgba(255,77,109,0.20)');
        ...
        >
          {status === 'error' ? (
            ...
            >
              ...
              <h3 className="text-base font-bold mb-1" style={{ color: '#f9fafb' }}>Stream Error</h3>
              <p className="text-xs mb-4" style={{ color: '#6b7280' }}>
                {errorMsg || 'Unable to connect to market data stream.'}
              </p>
              <button
                onClick={onRetry}
                className="inline-flex items-center gap-2 font-semibold text-xs px-4 py-2.5 rounded-xl"
            ...
            <>
              ...
              <p className="text-xs font-medium" style={{ color: '#4b5563' }}>
                {status === 'connecting' ? 'Opening stream…' : 'Loading chart…'}
              </p>
            </>
          )}
        </div>
...
export default function ChartsSection() {
  const [selected,   setSelected]   = useState<MarketItem>(MARKETS[4]); // 1HZ100V default
  ...
  // ── WebSocket lifecycle ────────────────────────────────────────────────────
  // RULE 1: Entire WS instantiation lives inside useEffect — runs in the browser only.
  useEffect(() => {
    let ws: WebSocket | null = null;
    let dead = false;           // guards setState after cleanup
    let usingFallback = false;

    // Reset view state whenever the selected asset or retry changes
    setTicks([]);
    setPrice(null);
    setPrevPrice(null);
    setChange(0);
    ...
    function openSocket(endpoint: string) {
      ...
      ws.onopen = () => {
        ...
        // Step A — preload 200 historical ticks so the chart renders instantly
        ...
        // Step B — RULE 3: Exact subscription payload per spec
        ws!.send(JSON.stringify({
      ...
      ws.onmessage = (ev: MessageEvent) => {
        ...
        try {
          ...
          console.error('[AutoTrendX] WS JSON parse error:', parseErr);
          return;
        }

        // RULE 5: Broker errors — log exact code + message, set UI state
        if (data.error) {
          console.error(
            `[AutoTrendX] Broker error — code: "${data.error.code}" | message: "${data.error.message}"`,
            data.error,
          );
          setStatus('error');
        ...
        // Historical payload from ticks_history request
      ...
      // onerror fires just before onclose — log it, let onclose handle fallback
      ws.onerror = (e) => {
        if (dead) return;
        console.error('[AutoTrendX] WebSocket error event:', e);
      };

      ws.onclose = (e) => {
        if (dead) return;
        console.warn(`[AutoTrendX] WS closed — code: ${e.code}, reason: "${e.reason}"`);

        // Intentional close (cleanup sent code 1000) — do nothing
        if (e.code === 1000) return;

        // Try fallback endpoint once before showing error
        if (!usingFallback && endpoint === WS_PRIMARY) {
          usingFallback = true;
          console.info('[AutoTrendX] Primary endpoint failed. Switching to fallback…');
          openSocket(WS_FALLBACK);
        } else {
          setStatus('error');
          setErrorMsg(`Connection closed unexpectedly (code ${e.code}). Click Retry.`);
        }
      };
    }
    ...
    // RULE 4: Cleanup — tell broker to forget all tick streams, then close cleanly.
    // This runs on every asset switch AND on component unmount.
    return () => {
      dead = true;
      if (ws) {
        if (ws.readyState === WebSocket.OPEN) {
          ws.send(JSON.stringify({ forget_all: 'ticks' }));
          ws.close(1000, 'Asset switch / component unmount');
        } else {
          ws.close();
        }
      }
  ...
  // ^^^ Deps: re-run the whole lifecycle when asset changes or user hits Retry.
  ...
  return (
    <div className="flex flex-col w-full" style={{ height: 'calc(100vh - 7rem)', background: '#0b0f1a' }}>
      ...
      >
        ...
        >
          ...
          <div className="text-left">
            ...
            <p className="text-xs" style={{ color: '#6b7280' }}>{selected.symbol}</p>
          </div>
          <ChevronDown
            size={14}
            style={{ color: '#6b7280' }}
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-xs" style={{ color: '#6b7280' }}>{statusLabel}</span>
            </div>
          </div>
        </div>
            ...
            >
              ...
              {favs.size > 0 && (
                <div className="px-3 pt-2">
                  <p className="text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 mb-1" style={{ color: '#4b5563' }}>
              ...
              <div className="flex-1 overflow-y-auto px-2 py-2">
                ...
                {Object.entries(grouped).map(([cat, items]) => (
                  <div key={cat} className="mb-2">
                    <p className="text-[10px] font-bold px-2 py-1 uppercase tracking-wider" style={{ color: '#374151' }}>
                      {cat}
                    </p>
                    {items.map(m => (
