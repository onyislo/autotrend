...
const FALLBACK = 'wss://ws.binaryws.com/websockets/v3?app_id=1089';
const MAX_TICKS = 200;
...
function MRow({ m, active, fav, onSelect, onFav }: {
  ...
  return (
    <div onClick={onSelect}
      className={`flex items-center gap-2 px-2 py-2 rounded-lg cursor-pointer transition-colors group ${active ? 'bg-blue-50' : 'hover:bg-gray-50'}`}>
      ...
      <button onClick={e => { e.stopPropagation(); onFav(); }} className="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
...
function ChartCanvas({ ticks, chartType, status, errorMsg, onRetry }: {
  ...
  const draw = useCallback(() => {
    ...
    for (let i = 0; i <= 6; i++) {
      ...
      ctx.fillText((max - (i/6)*range).toFixed(2), W-PR+6, y+4);
    }
    // Time labels
    ...
    [0,1,2,3,4,5,6].forEach(i => {
      ...
      ctx.fillText(d.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit',second:'2-digit'}), sx(idx), H-6);
    });
    ...
    if (chartType !== 'candle') {
      ...
      for (let b=0; b<BC; b++) {
        ...
        const mx=sx(Math.floor((b*bs+(b+1)*bs)/2)), clr=c>=o?'#00a500':'#ff4449';
        ctx.strokeStyle=clr; ctx.lineWidth=1;
        ctx.beginPath(); ctx.moveTo(mx,sy(hi)); ctx.lineTo(mx,sy(lo)); ctx.stroke();
        ctx.fillStyle=clr;
        ctx.fillRect(mx-bw/2,sy(Math.max(o,c)),bw,Math.max(1,Math.abs(sy(o)-sy(c))));
      }
    }
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 p-4">
          {status === 'error' ? (
            <div className="bg-red-50 border border-red-200 rounded-2xl p-6 max-w-sm text-center">
              ...
              <h3 className="text-base font-bold text-gray-900 mb-1">Connection Error</h3>
              <p className="text-xs text-gray-600 mb-4">{errorMsg || 'Unable to connect to market data.'}</p>
              <button onClick={onRetry} className="inline-flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white font-semibold text-xs px-4 py-2.5 rounded-xl">
...
export default function ChartsSection() {
  const [selected, setSelected] = useState<MarketItem>(MARKETS[4]); // Volatility 100 (1s)
  ...
  const connect = useCallback((item: MarketItem, fallback = false) => {
    setTicks([]); setPrice(null); setChange(0); setStatus('connecting'); setErrorMsg('');
    if (wsRef.current) { wsRef.current.close(); wsRef.current = null; }
    const ws = new WebSocket(fallback ? FALLBACK : WS_URL);
    wsRef.current = ws;
    ws.onopen = () => { setStatus('live'); ws.send(JSON.stringify({ ticks: item.symbol, subscribe: 1 })); };
    ws.onerror = () => { if (!fallback) { connect(item, true); return; } setStatus('error'); setErrorMsg('Cannot connect to Deriv data stream.'); };
    ws.onclose = (e) => { if (e.code !== 1000) { setStatus('error'); setErrorMsg(`Connection closed (${e.code})`); } };
    ws.onmessage = (ev) => {
      try {
        const msg = JSON.parse(ev.data);
        if (msg.error) { setStatus('error'); setErrorMsg(msg.error.message); return; }
        if (msg.tick) {
          const p = msg.tick.quote as number, t = msg.tick.epoch as number;
          setPrice(p);
          setTicks(prev => {
  ...
  useEffect(() => { connect(selected); return () => { wsRef.current?.close(); }; }, [selected, retry, connect]);

  const isUp = change >= 0;
  const filtered = MARKETS.filter(m => m.name.toLowerCase().includes(search.toLowerCase()) || m.symbol.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div className="flex flex-col bg-white w-full" style={{ height: 'calc(100vh - 7rem)' }}>

      {/* Header */}
      <div className="flex items-center gap-3 px-3 py-2 bg-white border-b border-gray-200 shrink-0 flex-wrap">
        <button onClick={() => setPanelOpen(v => !v)}
          className="flex items-center gap-2 bg-white border border-gray-200 hover:border-gray-300 rounded-xl px-3 py-2 shadow-sm">
          <div className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center"><SparkIcon /></div>
        ...
        <div className="flex items-center gap-2">
          <p className="text-xl font-bold text-gray-900 font-mono">{price !== null ? price.toFixed(2) : '—'}</p>
          <div>
            <span className={`text-sm font-bold block ${isUp ? 'text-emerald-600' : 'text-red-500'}`}>
              {isUp ? '▲' : '▼'} {Math.abs(change).toFixed(3)}%
            </span>
            <div className="flex items-center gap-1">
              <span className={`w-1.5 h-1.5 rounded-full ${status==='live'?'bg-emerald-500 animate-pulse':status==='error'?'bg-red-500':'bg-yellow-400 animate-pulse'}`} />
              <span className="text-xs text-gray-400">{status==='live'?'Live':status==='error'?'Error':'Connecting…'}</span>
            </div>
          </div>
        </div>
        <div className="ml-auto flex items-center gap-1">
          {([['line','line',TrendingUp],['area','area',Activity],['candle','candle',BarChart2]] as const).map(([type,,Icon]) => (
            <button key={type} onClick={() => setChartType(type as ChartType)}
              className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${chartType===type?'bg-blue-50 text-blue-600':'text-gray-400 hover:bg-gray-50'}`}>
              <Icon size={15} />
            </button>
          ))}
        </div>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        {panelOpen && (
          <>
            ...
            <div className="absolute md:relative z-30 md:z-auto top-0 left-0 h-full w-[85vw] max-w-xs md:w-64 bg-white border-r border-gray-200 flex flex-col shadow-xl md:shadow-none">
              <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                <span className="text-sm font-bold text-gray-800">Markets</span>
                <button onClick={() => setPanelOpen(false)} className="text-gray-400 hover:text-gray-700 md:hidden"><X size={18} /></button>
              </div>
        ...
        <div className="flex-1 min-w-0 h-full">
          <ChartCanvas ticks={ticks} chartType={chartType} status={status} errorMsg={errorMsg} onRetry={() => setRetry(r => r+1)} />
        </div>
      </div>
    </div>
  );
